...
if !speed_limit! equ 1 (
	...
	if !init_speed! gtr 0 (
		set "dml=--max-download-limit=!init_speed!K"
	)
)
:: 下载列表判断
if "%~1"=="" (
...
echo #使用ANSI编码保存，格式：$name\$url>>!list_name!
cls& set /p shut=是否开启指定时间段下载完成后关机功能[0.不开启，1.开启，默认1]：
if "!shut!"=="" set shut=1
cls& echo #是否开启限速功能[0.不开启，1.开启]>AdmX.ini&& echo speed_limit=!speed_limit!>>AdmX.ini&& echo #默认限速值[KB/s]>>AdmX.ini&& echo init_speed=^0>>AdmX.ini&& echo #默认下载列表文件名>>AdmX.ini&& echo list_name=!list_name!>>AdmX.ini&& echo #是否开启指定时间段下载完成后关机功能[0.不开启，1.开启]>>AdmX.ini&& echo shut=!shut!>>AdmX.ini|| goto admxini
if !shut!==1 (
	echo 下载完成后关机功能已开启！请分别输入作用时段的开始和结束时间。如在3点到6点之间使用下载完成关机功能，分别在开始时间输入3、结束时间输入6即可。
	...
	echo #下载关机时间段开始时间>>AdmX.ini&& echo period_start=!period_start!>>AdmX.ini&& echo #下载关机时间段结束时间>>AdmX.ini&& echo period_end=!period_end!>>AdmX.ini|| goto admxini
)
cls& goto :eof
:: Aria2配置
:aria2conf
...
cls& set /p file-allocation=文件预分配[预分配速度：none^>falloc^>prealloc，NTFS磁盘建议使用falloc，默认none]：
...
echo #文件缓存, 需要1.16及以上版本>aria2.conf&& echo disk-cache=!disk-cache!M>>aria2.conf&& echo #文件预分配, 能有效降低文件碎片. 缺点是预分配时间较长,所需时间 none ^> falloc ^> prealloc, falloc和trunc需要文件系统和内核支持>>aria2.conf&& echo file-allocation=!file-allocation!>>aria2.conf&& echo #追加HTTP请求头，用于迅雷离线下载验证>>aria2.conf&& echo header=Cookie: gdriveid=>>aria2.conf&& echo #断点续传>>aria2.conf&& echo continue=true>>aria2.conf&& echo #同服务器连接数>>aria2.conf&& echo max-connection-per-server=!max-connection-per-server!>>aria2.conf&& echo #单文件最大线程数>>aria2.conf&& echo split=!split!>>aria2.conf&& echo #最小文件分片大小, 下载线程数上限取决于能分出多少片, 对于小文件重要>>aria2.conf&& echo min-split-size=2M>>aria2.conf|| goto aria2conf
cls& goto :eof
:: 错误信息
:error0
